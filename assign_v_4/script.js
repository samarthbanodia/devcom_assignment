...
getExchangeRateButton.addEventListener("click", e=> { //get rates when button pressed
 ...
 REVERSE_SELECTION.addEventListener("click" ,()=> {   // swaps the from and to values 
     ...
     FetchExchangeRate(); // agains call excchange func
...
const amount = document.querySelector(".enter_amount input");
 ...
 if(amountValue == "0" || amountValue == ""|| amountValue == "00"|| amountValue == "000" || amountValue == "0000" || amountValue == "00000"){ //puts one if zero or no values
    amountValue = 1; // when user inputs 0,00,000,0000,00000 as input it sets amount as 1!!
    amount.value = "1";
  ...
  let API_URL = `https://v6.exchangerate-api.com/v6/4dc649a571d676489d6162a4/latest/${from_dropdown.value}`; //rates api
  ...
  fetch(API_URL).then(response =>  response.json().then(result => {
     ...
     // console.log(exchangeRate);
      ...
      const Accuracy_slider = document.querySelector(".slider input"); //slider for accuracy
      const Slider_value = parseInt(Accuracy_slider.value/10); // divide by 10 as 100 was defualt maxima 
      let Total_Rate_display  = (amountValue*exchangeRate).toFixed(Slider_value); //u can set accuract of conversion ; Total_rate is the final rate to be displayed
      //console.log(Total_Rate_display);
     // console.log(parseInt(Accuracy_slider.value/10));
 
      

      const Rate_text_display = document.querySelector(".output_rate p"); //displaying the TOtal_rate_dispaly on the page , in the p elemtn inside output_rate div
      if(isNaN(amountValue)){ // when amount is not a num ie wrong input 
...
function Flag_Loader(element){  //loads flag image whenver called
